...
class PreCleanUp:
      def __init__(self):
          ...
          #download the script
          cmd_boot_cleaner="cd /tmp;wget -O /tmp/bootSpace_clear.py %s;chmod +x /tmp/bootSpace_clear.py" %cmd_boot_space_cleaner_url
...
class PreChecksVM:
    ...
    def get_uek(self):
        cmd='wget -O /tmp/get_host_uek.py -q http://pd-yum-slc-01.us.oracle.com/data_files/oit/scripts/get_host_uek.py;python /tmp/get_host_uek.py'

        cmd_noproy='wget -O /tmp/get_host_uek.py -q --no-proxy --no-check-certificate http://pd-yum-slc-01.us.oracle.com/data_files/oit/scripts/get_host_uek.py;python /tmp/get_host_uek.py'

        (uek_status,uek_output) = commands.getstatusoutput(cmd)
  
        if uek_status == 0:
           return uek_output.split('|')[2]
        else:
...
#        yum_repo_file="/root/eng_ol%s.repo" %majversion
        ...
        if status != 0:
           download_repo = 'wget -O /tmp/generate_vm_repo.py --no-proxy --no-check-certificate http://pd-yum-slc-01.us.oracle.com/data_files/oit/scripts/repos/generate_vm_repo.py;chmod +x /tmp/generate_vm_repo.py;/tmp/generate_vm_repo.py'
           status, result = commands.getstatusoutput(download_repo)
