...
class PreCleanUp:
      ...
      def remove_older_kernels(self, stay):
          if os.path.exists('/tmp/bootSpace_clear.py'):
             (state,res) = commands.getstatusoutput('python /tmp/bootSpace_clear.py -kc %s' %stay)
             if not state:
      ...
      def find_remove_stale_kdumps(self):
          show_kernel_cmd="rpm -qa --last kernel kernel-uek | awk '{print $1}'"
          (show_kernel_status,show_kernel_output)=commands.getstatusoutput(show_kernel_cmd)
          if show_kernel_status == 0:
             kernel_list=show_kernel_output.split('\n')
          else:
             logging.error('Failed to get the installed kernels')
             return 
      
      
          #get the kdump files from /boot dir
          os.chdir('/boot')
          kdump_files = glob.glob('/boot/*kdump.img')
      
      
          #Filter for kdump where kernel does not exists
          exists_kdump=[]
          for kernel in kernel_list:
          	for kdump_file in kdump_files:
                 if kernel.split("-",2)[2] in kdump_file:
                    exists_kdump.append(kdump_file)
      
          remove_kdumps=set(kdump_files) - set(exists_kdump)
      
          if not len(remove_kdumps):
             return ("No kumps found to remove")
             logging.info("No kumps found to remove")

          for kdump in remove_kdumps:
              print('Removing %s' %kdump)
              logging.info('Removing %s' %kdump)
              os.system('rm -rf %s' %kdump)
...
class PreChecksVM:
    ...
    def run_all(self):
        ...
        RootFSState = self.root_space_checker()
         ...
         #remove stale kdumps file from /boot dir
        pc.find_remove_stale_kdumps()
