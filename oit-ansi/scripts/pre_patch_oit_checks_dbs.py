...
def kill_older_precheck_process():
    ...
    old_process_exists="ps -e -o 'pid,etime,command' | awk '{if($2>7200) print $0}' | grep pre_patch_oit_checks"
    old_process,_= commands.getstatusoutput(old_process_exists)

    if old_process == 0:
       kill_old_process="ps -e -o 'pid,etime,command' | awk '{if($2>7200) print $0}' | grep pre_patch_oit_checks | awk '{print $1}' | xargs kill -9"
