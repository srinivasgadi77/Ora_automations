    ...
    if os_type == 'DOMU':
       ...
       if os.path.exists(grub_file)  and os.path.getsize(grub_file) > 0:
          ...
          if post_cve_output:
             ...
             return "GrubFileDoesNotExits","NO"

    elif os_type == 'DOM0':
        if os.path.exists('/boot/grub2/grubenv'):
           cmd="grep %s /boot/grub2/grubenv" %get_latest_kernel
           #cmd="grep saved_entry  /boot/grub2/grubenv | awk '{print $NF}'"
           (default_grub_status,default_grub_output) = commands.getstatusoutput(cmd)
           if default_grub_status == 0 and default_grub_output:
              return "GrubSetWithLatestKernel",get_latest_kernel
           else:
             return "GrubNOTSetWithLatestKernel","NO"
        else:
...
def get_initrm():
    ...
    if os_type == 'DOMU':
       ...
       cmd = "ls /boot |grep %s |grep -v debug |grep -v ksplice|grep -i init" %get_latest_kernel
...
if not check_if_patch_running():
   ...
   if 'NotPatched' in patch_status or 'OL5' in patch_status:
      print("%s|%s" %(get_uek(),patch_status))
   else:  
      get_uek_ver=get_uek()
      if get_uek_ver != 0:
         ...
         print('%s,%s,%s,%s,%s' %(get_uek(),patch_status,get_defaukt_grub_status,get_initrm(),reboot_status))
      else:
         print('%s,%s,%s,%s,%s' %(get_uek(),patch_status,'NonUEK','NA','reboot_required')) 
else:
