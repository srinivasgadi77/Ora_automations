...
def executeWithkeys(host, username, command, isFile, results,AuthType, PasswdValue):
    ...
    try:
        ...
        try:
            print("LogInSuccess: %s => %s" %(host,pvalue))
            ...
            print("====> os_type_err => %s" %os_type_err)

            if "No space left on device" in os_type_err:
                write_to_db(PID, 'Error',host,'No space left on device')
                return True
            ...
            if isFile:
                try:
                    ...
                    print("===>",os.path.join(source_folder,file1),os.path.join(localpath,file1))
            ...
            if username  == 'root':
                ...
                print('Running with OPC')
                #import pdb;pdb.set_trace()
                given_cmd_stdin, given_cmd_out, given_cmd_stderr= ssh.exec_command(f"sudo su - root")
                given_cmd_stdin, given_cmd_out, given_cmd_stderr= ssh.exec_command(f"id")
                print("======> %s"%given_cmd_out.read())
                given_cmd_stdin, given_cmd_out, given_cmd_stderr= ssh.exec_command(f"{command_exe}")
            else:
                print(f'\n\t=> with {username}')
...
def manual_execution(host,username,passwords,command,isFile):
    ...
    state = executeWithkeys(host, LDAP,  command, isFile, results, 'Ptype', passwords[-1])
...
def main():
    ...
    if args.enable_passwords:
        ...
        #passwords = ['HuNAb_Ku', '$hArk13$', 'welcome1', 'S1s1lian!', 'S1s1lian','Thatsn0t1t!', 'DangEdd4','S41v1@n3', 'L0ck!tup', 'welecome123', '0pnW0r1d', 'D1s@P3&0', 'welcome', 'L@n_B$cK7', '$t33L3R$', '$h@rk13$', 'Ca8ra_Ka','r00t06','L@n_B$cK7','Sy7vi@n$_p','K@z^h1r0']
        passwords = ['welcome1']
