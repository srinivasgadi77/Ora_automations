...
def verify_default_grub():
    ...
    if os.path.exists(grub_file)  and os.path.getsize(grub_file) > 0:
       ...
       if post_cve_output:
          return "GrubSetWithLatestKernel"
       else:
          return "GrubNOTSetWithLatestKernel"
def get_initrm():
...
if not check_if_patch_running():
   ...
   if 'NotPatched' in patch_status:
      print(patch_status)
   else:
      print('%s,%s,%s' %(patch_status,verify_default_grub(),get_initrm()))
else:
